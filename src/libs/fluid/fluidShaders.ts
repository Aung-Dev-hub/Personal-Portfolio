...
vec3 linearToGamma (vec3 color) {
  ...
  return max(
    1.055 * pow(color, vec3(0.416666667))
      - 0.055,
    vec3(0)
  );
}
...
void main () {
  ...
  vec3 n = normalize(
    vec3(dx, dy, length(texelSize))
  );

  vec3 l = vec3(0.0, 0.0, 1.0);

  float diffuse =
    clamp(dot(n, l) + 0.7, 0.7, 1.0);
