...
export default function useFluidCanvas(
    ...
    useEffect(() => {
        /*
         * IMPORTANT:
         * Keep a local non-null canvas reference.
         *
         * This fixes:
        ...
        const bloomBlurProgram =
            makeProgram(
                baseVertex,
                bloomBlurFrag
            );

        const bloomFinalProgram =
            makeProgram(
                baseVertex,
                bloomFinalFrag
            );

        const sunraysMaskProgram =
            makeProgram(
                baseVertex,
                sunraysMaskFrag
            );

        const sunraysProgram =
            makeProgram(
                baseVertex,
                sunraysFrag
            );
        ...
        let displayProgram:
            FluidProgram | null = null;

        function getDisplayProgram() {
            const keywords: string[] = [];

            if (config.SHADING) {
                keywords.push("SHADING");
            }
        ...
        const ditheringTexture =
            (() => {
                ...
                if (!texture) {
                    throw new Error(
                        "Failed to create dithering texture."
                    );
                }
        ...
        function render(
            ...
            if (config.BLOOM) {
                gl.uniform1i(
                    displayProgram.uniforms.uBloom,
                    bloom.attach(1)
                );

                gl.uniform1i(
                    displayProgram.uniforms.uDithering,
                ...
                const scale = {
                    x:
                        (target == null
                            ? gl.drawingBufferWidth
                            : target.width) /
                        ditheringTexture.width,

                    y:
                        (target == null
                            ? gl.drawingBufferHeight
                            : target.height) /
                        ditheringTexture.height,
                };
            ...
            if (config.SUNRAYS) {
                gl.uniform1i(
                    displayProgram.uniforms.uSunrays,
                    sunrays.attach(3)
                );
            }
